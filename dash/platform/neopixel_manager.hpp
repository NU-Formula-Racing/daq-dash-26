...
>>>>>>> a451bed7a769e20e549b2a9a0facf09bfb692cde
            }
        ...
        void bmsSocAnimation(float time) {
            ...
            if (numFull != bar2NumPixels) {
                glm::vec4 partialColor =
                    color * (batteryPercentage * getBar(2).numPixels() - numFull);
        ...
        */
        ...
        void socChargeAnimation(float time) {  // bar 1
            ...
            int numFull = static_cast<int>(
                floor(batteryPercentage *
                      getBar(1).numPixels()));  // this is the number that will be green
        ...
        void batteryTempAnimation(float time) {                   // bar 4
            ...
            float tempPercentage =
                (static_cast<float>(dbc::bmsDaughterboard::batteryTemperature->get()) -
                    lowerBound) /
                (upperBound - lowerBound);
        ...
        void neutral(float time) {
            ...
            for (int i = 0; i < 5; i++) {  // for all 5 bars
                if (i == 2)
                    continue;
                int barOffset = i * 2;
                for (int j = 0; j < getBar(i).numPixels();
                    j++) {  // this indexes the leds on each bar
