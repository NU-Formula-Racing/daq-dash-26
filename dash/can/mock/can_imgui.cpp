...
bool CAN_IMGUI::recv(CAN_Frame& msg) {
    ...
    switch (info.changedSignal.type) {
        case SignalType::INT8:
            signal->set(info.changedSignal.type, &info.changedSignal.value.s8);
            break;
        case SignalType::INT16:
            signal->set(info.changedSignal.type, &info.changedSignal.value.s16);
            break;
        case SignalType::INT32:
            signal->set(info.changedSignal.type, &info.changedSignal.value.s32);
            break;
        case SignalType::INT64:
            signal->set(info.changedSignal.type, &info.changedSignal.value.s64);
            break;
        case SignalType::UINT8:
            signal->set(info.changedSignal.type, &info.changedSignal.value.u8);
            break;
        case SignalType::UINT16:
            signal->set(info.changedSignal.type, &info.changedSignal.value.u16);
            break;
        case SignalType::UINT32:
            signal->set(info.changedSignal.type, &info.changedSignal.value.u32);
            break;
        case SignalType::UINT64:
            signal->set(info.changedSignal.type, &info.changedSignal.value.u64);
            break;
        case SignalType::FLOAT:
            signal->set(info.changedSignal.type, &info.changedSignal.value.f);
            break;
        case SignalType::BOOL:
            signal->set(info.changedSignal.type, &info.changedSignal.value.b);
            break;
    }
            ...
            if (tabOpen) {
                ...
                if (ImGui::CollapsingHeader(msgName, ImGuiTreeNodeFlags_DefaultOpen)) {
                    ...
                    for (uint8_t sigNum{}; sigNum < message->get_num_signals(); sigNum++) {
                        ...
                        ImGui::PushItemWidth(
                            -(ImGui::CalcTextSize(name).x + ImGui::GetStyle().ItemInnerSpacing.x));

                        switch (signal->getSignalType()) {
                            case SignalType::INT8: {
                                auto s = static_cast<CAN_Signal<int8_t>*>(signal);
                                sigInfo.value.s8 = s->get();
                                if (ImGui::InputScalar(name, ImGuiDataType_S8, &sigInfo.value.s8))
                                    changed = true;
                                break;
                            }
                            case SignalType::INT16: {
                                auto s = static_cast<CAN_Signal<int16_t>*>(signal);
                                sigInfo.value.s16 = s->get();
                                if (ImGui::InputScalar(name, ImGuiDataType_S16, &sigInfo.value.s16))
                                    changed = true;
                                break;
                            }
                            case SignalType::INT32: {
                                auto s = static_cast<CAN_Signal<int32_t>*>(signal);
                                sigInfo.value.s32 = s->get();
                                if (ImGui::InputScalar(name, ImGuiDataType_S32, &sigInfo.value.s32))
                                    changed = true;
                                break;
                            }
                            case SignalType::INT64: {
                                auto s = static_cast<CAN_Signal<int64_t>*>(signal);
                                sigInfo.value.s64 = s->get();
                                if (ImGui::InputScalar(name, ImGuiDataType_S64, &sigInfo.value.s64))
                                    changed = true;
                                break;
                            }
                            case SignalType::UINT8: {
                                auto s = static_cast<CAN_Signal<uint8_t>*>(signal);
                                sigInfo.value.u8 = s->get();
                                if (ImGui::InputScalar(name, ImGuiDataType_U8, &sigInfo.value.u8))
                                    changed = true;
                                break;
                            }
                            case SignalType::UINT16: {
                                auto s = static_cast<CAN_Signal<uint16_t>*>(signal);
                                sigInfo.value.u16 = s->get();
                                if (ImGui::InputScalar(name, ImGuiDataType_U16, &sigInfo.value.u16))
                                    changed = true;
                                break;
                            }
                            case SignalType::UINT32: {
                                auto s = static_cast<CAN_Signal<uint32_t>*>(signal);
                                sigInfo.value.u32 = s->get();
                                if (ImGui::InputScalar(name, ImGuiDataType_U32, &sigInfo.value.u32))
                                    changed = true;
                                break;
                            }
                            case SignalType::UINT64: {
                                auto s = static_cast<CAN_Signal<uint64_t>*>(signal);
                                sigInfo.value.u64 = s->get();
                                if (ImGui::InputScalar(name, ImGuiDataType_U64, &sigInfo.value.u64))
                                    changed = true;
                                break;
                            }
                            case SignalType::FLOAT: {
                                auto s = static_cast<CAN_Signal<float>*>(signal);
                                sigInfo.value.f = s->get();
                                if (ImGui::InputFloat(name, &sigInfo.value.f))
                                    changed = true;
                                break;
                            }
                            case SignalType::BOOL: {
                                auto s = static_cast<CAN_Signal<bool>*>(signal);
                                sigInfo.value.b = s->get();
                                if (ImGui::Checkbox(name, &sigInfo.value.b))
                                    changed = true;
                                break;
                            }
                        }
                        ...
                        if (changed) {
                            result = okay::Option<CAN_IMGUI::MessageChangeInfo>::some(
                                {messageID, sigNum, sigInfo});
                        }
                    }
